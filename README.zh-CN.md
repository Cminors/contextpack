...
| SWE-bench Axios issue | 6 | 0.617 | 0.177 | 文件级真实 issue 冒烟 |

Axios 冒烟在 100、250、500 行预算下的行召回均为 `0.000`。这说明当前版本有时能找到正确文件，但还不能稳定定位到文件中的正确代码区域。因此项目仍保持“源码预览版”状态，不对 Agent 成功率作宣传性承诺。
...
ContextPack 目前是未发布的实验性源码预览版：核心 CLI 可以运行，打包结构、自动测试、性能烟测和真实 issue 评测已经具备，但 npm 发布、正式 Release、零基础安装体验和文件内行定位仍未完成。
...
当前质量门禁：60 项测试通过、行覆盖率超过 88%、生产依赖漏洞为 0，并包含一个确定性的 360 文件性能烟测。GitHub CI 验证 Node.js 20 和 22。
