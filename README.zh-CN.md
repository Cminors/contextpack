...
P1.0 把语言相关的文件发现和源码分析放到一个小型的内部适配器边界之后。确定性的注册表提供源码和配置文件模式，验证每个发现的源码文件恰好有一个所有者，并按稳定顺序分派分析。现有 JavaScript/TypeScript 适配器继续生成规范化的 `FileAnalysis` 数据，供与语言无关的排序、区域选择和渲染流水线使用；兼容门面保留原有分析 API 和 manifest 格式。

这是内部实现边界，不是插件或公开 SDK 接口。ContextPack 仍只支持 JavaScript 和 TypeScript 项目；适配器架构不表示已经提供其他语言或集成支持。
