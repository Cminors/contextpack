...
P1.1 把语言相关的文件发现和源码分析放到一个小型的内部适配器边界之后。确定性的注册表提供源码和配置文件模式，验证每个发现的源码文件恰好有一个所有者，并按稳定顺序分派分析。JavaScript/TypeScript 使用现有的编译器适配器；Python 使用 Python 3.8+ 标准库 `ast` worker，没有解释器时使用受控的词法回退。两者都生成规范化的 `FileAnalysis` 数据，供与语言无关的排序、区域选择和渲染流水线使用；兼容门面保留原有分析 API 和 manifest 格式。

这是内部实现边界，不是插件、Skills、MCP 或公开 SDK 接口。Python 支持源码发现、顶层符号、仓库内部导入、测试/配置分类、Python 注释和字符串，以及有证据的验证命令；不表示支持框架语义或完整类型推断。
...
- JavaScript、JSX、TypeScript、TSX、MJS、CJS、MTS、CTS 和 Python 3.8+；
