...
P1.1 把语言相关的文件发现和源码分析放到一个小型的内部适配器边界之后。确定性的注册表提供源码和配置文件模式，验证每个发现的源码文件恰好有一个所有者，并按稳定顺序分派分析。JavaScript/TypeScript 使用现有的编译器适配器；实验性的 Python 适配器使用 Python 3.8+ 标准库 `ast` worker，没有解释器时使用受控的词法回退。两者都生成规范化的 `FileAnalysis` 数据，供与语言无关的排序、区域选择和渲染流水线使用；兼容门面保留原有分析 API 和 manifest 格式。

这是内部实现边界，不是插件、Skills、MCP 或公开 SDK 接口。实验性的 Python 适配器支持源码发现、顶层符号、仓库内部导入、测试/配置分类、Python 注释和字符串，以及有证据的验证命令。由于四个实例持续超时，完整真实 issue 基准运行无效，所以 Python 检索支持尚未得到验证；这也不表示支持框架语义或完整类型推断。
...
- 手上已经有一个 JavaScript、TypeScript、Python 或混合 JS/Python 项目；
- 想减少 AI 在大仓库里盲目搜索的人；
- 愿意测试实验性工具并反馈问题的人。
...
当前目录中没有受支持的 JavaScript、TypeScript 或 Python 文件。确认你已经 `cd` 到正确的项目根目录，而不是 ContextPack 自己的目录、桌面目录或某个空文件夹。
...
- JavaScript、JSX、TypeScript、TSX、MJS、CJS、MTS 和 CTS，以及实验性的 Python 3.8+ 适配器；
- Python 顶层函数、异步函数、类、方法、变量、仓库内部导入、pytest 风格测试文件，以及常见打包/配置文件；
- 纯 Python 和混合 JavaScript/Python 仓库；Python 不可用时会使用词法回退；
...
- 自动修改代码或自动运行 Agent；
- Go、Rust、Java 等语言；
- Python 框架语义、完整类型推断、动态导入和语义调用/引用图；
- 经过基准验证的 Python 真实 issue 检索；
- 公开语言插件 API、Skills 和 MCP 集成；
...
| SWE-bench JS/TS P1.0 parity 运行 | 43 个有效 / 43 个已尝试 | 0.389 | 0.177 | 与 P0.9 的预测逐项一致 |
| SWE-bench Lite Python 工程集 | 57 个有效 / 57 个已尝试 | 0.474 | 0.188 | 零跳过基础设施门，不是支持结论 |
| SWE-bench Lite Python P1.2 完整运行 | 296 个有效 / 300 个已尝试 | 0.311 | 0.126 | **invalid-run：**4 个 SymPy 实例持续超时 |

P1.2 没有验证 Python 真实 issue 检索。固定的完整运行在允许的重试后仍保留 4 个跳过实例，因此确定性判定为 `invalid-run`。在 296 个有效实例上，500 行预算的行召回为 `0.066718`，但 useful hit @500 为 `0.097973`，低于预先声明的 `0.100` 门槛。这些部分指标只能用于诊断，不能证明文件或区域检索支持，也不衡量代码生成或补丁正确性。完整证据见 [P1.2 Benchmark](benchmarks/README.md#p12-python-benchmark)。
